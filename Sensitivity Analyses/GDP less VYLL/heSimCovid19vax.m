function [f,g,tvec,h]=heSimCovid19vax(pr,vx,beta,tvec,Dvec,n,ntot,NNvec,phi1,phi2,seedvec,S0,tau,plotTau)
%% PARAMETERS:
...
if solvetype==2
    ...
    for i=1:lt-1
        ...
        [tout,Sclass,Hclass,Dclass,DEcum,Rcum,Itot,y0,Hnew,Sv1class,Vclass,ISclass]=integr8(pr,vx,beta,ntot,NNfeed,D,phi1,phi2,seedvec,t0,tend,y0,i,hospInc);
        ...
        DEout(i,1:4)=[DEcum(64),DEcum(65),sum(DEcum([1:63,66])),DEcum(67)];
        %Rout(:,i)=Rcum;
        Vout=[Vout;Vclass(2:end,:)];
        ISout=[ISout;ISclass(2:end,:)./repmat(NNfeed',size(ISclass,1)-1,1)];
        HHout=[HHout;Hclass(2:end,:)./repmat(NNfeed',size(Hclass,1)-1,1)];
        DDout=[DDout;Dclass(2:end,:)./repmat(NNfeed',size(Dclass,1)-1,1)];
        IStot=[IStot;sum(ISclass(2:end,[1:63,66]),2)/sum(NNfeed([1:63,66]))];
        HHtot=[HHtot;sum(Hclass(2:end,[1:63,66]),2)/sum(NNfeed([1:63,66]))];
        DDtot=[DDtot;sum(Dclass(2:end,[1:63,66]),2)/sum(NNfeed([1:63,66]))];
...
function [tout,Sclass,Hclass,Dclass,DEcum,Rcum,Itot,y0new,Hnew,Sv1class,Vclass,ISclass]=integr8(pr,vx,beta,ntot,NN0,D,phi1,phi2,seedvec,t0,tend,y0,topen,hospInc)
%% CALL:
