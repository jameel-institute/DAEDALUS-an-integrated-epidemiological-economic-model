function [f,g,tvec,h]=heRunCovid19(pr,vx,n,ntot,na,NN,NNbar,NNrep,Dout,beta,Xit,tvec,plotTau,data)%,pmodIn)
...
NNvec=[NNbar,[NNbar(1:lx).*data.xmin';NNbar(lx+1:lx+lc)],NNvec];
...
%
Dvec=repmat(D,[1,1,lt-1]);
Dvec(:,:,2)=pr.betamod(2)*heMakeDs(NNvec(:,2),data.xmin',data,1);%,0); %NNfrac
%if lt>3
...
[DEout,Rout,tvec,h]=heSimCovid19vax(pr,vx,beta,tvec,Dvec,n,ntot,NNvec,phi1,phi2,seedvec,NNvec(:,1),t,plotTau);%S0=NNbar (2nd last arg)
