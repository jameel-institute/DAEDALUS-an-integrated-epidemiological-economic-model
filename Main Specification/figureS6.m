...
for i=11:-1:1
    my_field = strcat('h',num2str(i));
    variable.(my_field) = plot(h(:,2*i-1),h(:,2*i),'-','linewidth',lw,'color',cmap(i,:));
end
...
grid on;
grid minor;
legend([variable.h1,variable.h6,variable.h11],'0% reduction','10% reduction','20% reduction','location','northwest');
...
for i=10:-1:1
    my_field = strcat('h',num2str(i));
    variable.(my_field) = plot(hh(:,2*i-1),hh(:,2*i),'-','linewidth',lw,'color',cmap(i,:));
end
